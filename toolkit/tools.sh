...
_mount_remote() {
    sudo mount -t cifs -o username="${_username}",password="${_password}",uid="${_uid}",gid="${_gid}",dir_mode=0755,file_mode=0644,port=44555 "${@}"
}
...
backup() {
    if [ -z "${1-}" ] || [ -z "${2-}" ] || [ ! -b "${1}" ]
    then
        echo "Usage example: backup /dev/nvme0n1 /home/igor/private/archive/backups/win" >&2
        return 1
    fi
...
restore() {
    if [ -z "${1-}" ] || [ -z "${2-}" ] || [ ! -b "${2}" ]
    then
        echo "Usage example: restore /home/igor/private/archive/backups/win.img.gz /dev/nvme0n1" >&2
        return 1
    fi
    gunzip -c "${1}" | dd of="${2}"
}
