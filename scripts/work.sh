...
# WinXP
# The server security layer detected an error (0x80090304) in the protocol stream and the client (Client IP: 192.168.1.XX) has been disconnected.
...
_info "Connecting to: ${_user} @ ${_domain}/${_host}"
exec xfreerdp3 /cert:ignore /bpp:32 /network:lan /audio-mode:0 /sound:sys:pulse /scale:180 /monitors:0 /floatbar:sticky:off /gfx:RFX /log-level:FATAL /timeout:30000 +f -themes -wallpaper -grab-keyboard \
/d:"${_domain}" \
/v:"${_host}" \
/u:"${_user}" \
/p:"${_password}"
